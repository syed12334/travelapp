...
use App\Http\Requests\Admin\Pamenities;
use App\Services\Admin\PropertyamenityService;

class PropertyamenityController extends Controller
{
    protected $pamenity;
    public function __construct(PropertyamenityService $pmser) {
        $this->pamenity = $pmser;
    }
    /* Amenity index */ 
    public function index(Request $request) {
        ...
        if($request->ajax()) {
            return response()->json([
                'status' =>true,
                'html' => view('backed.property_amenity.pamenitydata', compact('amenity'))->render(),
            ]);
        }
        return view('backed.property_amenity.pamenities',compact('amenity'));
    }
    /* Store amenity */
    public function store(Pamenities $req) {
