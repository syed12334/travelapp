...
use App\Repositories\Admin\PropertyamenityRepository;

class PropertyamenityService
{
    protected $pamenity;
    public function __construct(PropertyamenityRepository $am)
    {
        $this->pamenity = $am;
    }
    /* Fetch all service */
    ...
    public function store($req) {
        $req->validated();
         $d['pamenity_uuid'] = Str::uuid();
        $d['pamenity_text'] = $req['amenity'];
    ...
    public function update($request) {
        ...
        $data['pamenity_text'] = $amenity;
        $this->pamenity->updateAmenityById($amenity_id,$data);
        return response()->json([
            'status' =>true,
            'msg'    =>'Property amenity updated successfully'
        ]);
    }
    ...
    public function statusChange($request) {
        ...
        $data['status'] = $status;
        if($status ==0) {
            $msg = "Property amenity deactivated successfully";
        }
        else if($status ==1) {
             $msg = "Property amenity activated successfully";
        }
        else if($status ==2) {
             $msg = "Property amenity deleted successfully";
        }
        $this->pamenity->updateAmenityById($amenity_id,$data);
        return response()->json([
            'status' =>true,
    ...
    public function deleteMultipleamenity($mdelete) {
        if(isset($mdelete['amenity_id']) && !empty($mdelete['amenity_id'])) {
            ...
            return back()->with('success','Property amenities deleted successfully');
        }
    } 
}
