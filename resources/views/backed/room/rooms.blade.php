...
@endpush
 <form action="{{route('rooms')}}" method="get" id="getSubmitform">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="page-title">
                        <h4 id="modalTitle">Add Room Category</h4>
                    </div>
                ...
                <div id="errorMsg"></div>
                <form action="{{ route('rstore') }}" method="post" id="amenityForm">
                    <input type="hidden" name="room_id" id="room_id" >
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Enter room category<span class="text-danger ms-1">*</span></label>
                            <input type="text" class="form-control" name="room" id="name" placeholder="Enter room" required>
                            <span class="text-danger" id="error_room"></span>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn me-2 btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-primary" id="categorySubmitButton">Add Room</button>
...
$(function () {
 const $form           = $('#amenityForm');
  const listUrl         = "{{ route('rajaxroomlist') }}";
 const editUrl         = "{{ route('reditRoomlist') }}";
const updateUrl       = "{{ route('rupdateRoomlist') }}";

    /* ---------- jQuery‑Validate ---------- */
    $form.validate({
        rules:    { amenity: { required: true, minlength: 3 } },
        messages: { amenity: { required: 'Please enter a room',
                            minlength: 'Your room name must consist of at least 3 characters' } },
        ...
        $.post(editUrl, { id })
         .done(resp => {
             if (resp.status) {
                 ...
                 clearForm();
                 $('#name').val(c.rooms);                 // << proper key
                 $('#room_id').val(c.id);
                 $('#modalTitle').text('Edit Room');
                 $('#categorySubmitButton').text('Update Room');
                 $form.attr({ action: updateUrl, method: 'POST' });
                 $('#add-amenity').modal('show');
             } else {
                 toastr.error(resp.message || 'Unable to load room.');
             }
         })
         .fail(ajaxFail);
    };
