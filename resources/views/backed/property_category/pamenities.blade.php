...
@endpush
 <form action="{{route('pcamenities')}}" method="get" id="getSubmitform">
    ...
    <div class="content">
        <div class="page-header">
            <div class="add-item d-flex">
                <div class="page-title">
                    <h4 class="fw-bold">Property categories</h4>
                    <h6>Manage your property category</h6>
                </div>
            </div>
            <div class="page-btn">
                <a href="#" class="btn btn-primary clearform" data-bs-toggle="modal" data-bs-target="#add-amenity">
                    <i class="ti ti-circle-plus me-1"></i>Add Property Category
                </a>
            </div>
        </div>
        <!-- Category List -->
        <div class="card">
            ...
            <div class="card-body p-0" style="position:relative">
                <form action="{{ route('pcmultipleAmenityDelete') }}" method="post">
                    @csrf
                    <div class="card-trash">
                        <i class="fa fa-trash" title="Delete Amenities"></i>
                    </div>
                    <div class="table-responsive" id="amenityListtable">
                        @include('backed.property_category.pamenitydata',['amenity'=>$amenity]);
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="page-title">
                        <h4 id="modalTitle">Add Property Category</h4>
                    </div>
                ...
                <div id="errorMsg"></div>
                <form action="{{ route('pcastore') }}" method="post" id="amenityForm">
                    <input type="hidden" name="amenity_id" id="amenity_id" >
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
                            <label class="form-label">Enter amenity<span class="text-danger ms-1">*</span></label>
                            <input type="text" class="form-control" name="amenity" id="name" placeholder="Enter amenity" required>
                            <span class="text-danger" id="error_amenity"></span>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn me-2 btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-primary" id="categorySubmitButton">Add Amenity</button>
...
$(function () {
 const $form           = $('#amenityForm');
  const listUrl         = "{{ route('pcajaxamenitylist') }}";
 const editUrl         = "{{ route('pceditAmenitylist') }}";
const updateUrl       = "{{ route('pcupdateAmenitylist') }}";

    /* ---------- jQuery‑Validate ---------- */
    $form.validate({
        rules:    { amenity: { required: true, minlength: 3 } },
        messages: { amenity: { required: 'Please enter a amenity',
                            minlength: 'Your amenity name must consist of at least 3 characters' } },
        ...
        $.post(editUrl, { id })
         .done(resp => {
             if (resp.status) {
                 ...
                 clearForm();
                 $('#name').val(c.pcamenity_text);                 // << proper key
                 $('#amenity_id').val(c.id);
                 $('#modalTitle').text('Edit Amenity');
                 $('#categorySubmitButton').text('Update Amenity');
                 $form.attr({ action: updateUrl, method: 'POST' });
                 $('#add-amenity').modal('show');
             } else {
                 toastr.error(resp.message || 'Unable to load amenity.');
             }
         })
         .fail(ajaxFail);
    };
  ...
  var checkboxes = $('input[type="checkbox"]');
        checkboxes.change(function(){
            ...
            if(countCheckedCheckboxes >0) {
                $('#numberofcount').text(countCheckedCheckboxes);
                $('#datalist').val(countCheckedCheckboxes);
            }else {
                $("#player_status_text").text('Are you sure you want to activate selected player?');
                $('#datalist').val('');
                $('#numberofcount').text(0);
            }
        });
